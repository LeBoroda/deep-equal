...
export class ObjectComparator extends AbsComparator {
  ...
  compare(a:Record<string, unknown>, b:Record<string, unknown>, context:DeepEqualEngine):boolean {
    ...
    return keysA.every(
      (key:string):boolean => keysB.includes(key) && context.deepEqual(a[key], b[key]),
    );
  }
}
